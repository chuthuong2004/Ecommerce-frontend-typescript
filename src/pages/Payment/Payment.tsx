...
const Payment: React.FC<Props> = () => {
  ...
  return (
    <Helmet title="Thanh toán - Koga-clothes.shop">
                    ...
                    {user?.addresses &&
                      user?.addresses?.map((address: IAddressUser) => (
                        <option selected={address.isDefault} key={address._id} value={address._id}>
                          ...
                          {address.ward?.includes('Thị trấn') &&
                            address.ward.replace('Thị trấn', '')}
                          ,
                          {address.district?.includes('Quận') &&
                            address.district.replace('Quận', '')}
