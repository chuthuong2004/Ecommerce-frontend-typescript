import routes from '@/config/routes';

export const colLinks = [
  {
    title: 'Thông tin',
    links: [
      {
        to: routes.introduce,
        content: 'Giới thiệu Koga',
      },
      {
        to: '#',
        content: 'Hệ thống cửa hàng',
      },
      {
        to: '#',
        content: 'Tuyển dụng',
      },
      {
        to: '#',
        content: 'Thông tin liên hệ',
      },
      {
        to: '#',
        content: 'bộ công thương',
        srcImg: '/logoSaleNoti.png',
      },
    ],
  },
  {
    title: 'Trợ giúp',
    links: [
      {
        to: '#',
        content: 'Hướng dẫn tạo tài khoản',
      },
      {
        to: '#',
        content: 'Phương thức thanh toán',
      },
      {
        to: '#',
        content: 'Chính sách VIP',
      },
      {
        to: '#',
        content: 'Chính sách giao hàng',
      },
      {
        to: '#',
        content: 'Chính sách đổi trả',
      },
      {
        to: '#',
        content: 'Chính sách bảo mật',
      },
    ],
  },

  {
    title: 'Thanh toán',
    links: [
      {
        to: '#',
        content: 'Visa / Mastercard / JCB',
      },
      {
        to: '#',
        content: 'Internet Banking',
      },
      {
        to: '#',
        content: 'Quét mã QR',
      },
      {
        to: '#',
        content: 'Thanh toán khi nhận hàng (COD)',
      },
    ],
  },
  {
    title: 'Giao hàng',
    links: [
      {
        to: '#',
        content: 'Tiki Smart Logistics',
      },
      {
        to: '#',
        content: 'Ahamove',
      },
    ],
  },
];
