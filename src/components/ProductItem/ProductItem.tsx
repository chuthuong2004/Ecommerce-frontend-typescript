...
const ProductItem: React.FC<Props> = ({ product }) => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await productApi.getById(currentProduct._id);
        setCurrentProduct((prev: IProduct) => ({ ...prev, favorites: res.favorites }));
        setIsLiked(res?.favorites.includes(user?._id || ''));
      } catch (error) {
        console.log(error);
      }
    };
